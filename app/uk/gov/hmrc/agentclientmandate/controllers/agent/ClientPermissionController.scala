...
trait ClientPermissionController extends FrontendController with Actions {
  ...
  def view(service: String, backLinkUrl: Option[String]) = AuthorisedFor(AgentRegime(Some(service)), GGConfidence).async {
    implicit user => implicit request =>
      ...
      } yield Ok(views.html.agent.clientPermission(clientPermissionForm, service, backLinkUrl))
  }


  def submit(service: String, backLinkUrl: Option[String]) = AuthorisedFor(AgentRegime(Some(service)), GGConfidence) {
    implicit user => implicit request =>

    ...
    implicit authContext => implicit request =>
      dataCacheService.fetchAndGetFormData[ClientCache](clientFormId) map { a =>
        a.flatMap(_.mandate) match {
          case Some(x) => Ok(views.html.agent.searchPreviousMandate(service, mandateRefForm.fill(MandateReference(x.id)), getBackLink(service, callingPage)))
          case None => Ok(views.html.agent.searchPreviousMandate(service, mandateRefForm, getBackLink(service, callingPage)))
        }
      }
  }

//  def submit(service: String, callingPage: String) = AuthorisedFor(ClientRegime(Some(service)), GGConfidence).async {
//    implicit authContext => implicit request =>
//      mandateRefForm.bindFromRequest.fold(
//        formWithErrors => Future.successful(BadRequest(views.html.client.searchMandate(service, formWithErrors,  getBackLink(service, callingPage)))),

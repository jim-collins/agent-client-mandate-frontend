  ...
  def view(service: String, mandateId: String) = AuthorisedFor(AgentRegime, GGConfidence).async {
    implicit authContext => implicit request =>
      ...
      acmService.fetchClientMandate(mandateId).map { response =>
        response match {
          case Some(mandate) => Ok(views.html.agent.removeClient(new YesNoQuestionForm("agent.remove-client.error").yesNoQuestionForm, mandate.clientParty.get.name, service, mandateId))

...
class RejectClientControllerSpec extends PlaySpec with OneServerPerSuite with MockitoSugar with BeforeAndAfterEach {
  ...
  def viewWithAuthorisedAgent(test: Future[Result] => Any) {
    ...
    val result = TestRejectClientController.view("1").apply(SessionBuilder.buildRequestWithSession(userId))
    test(result)
  }
  ...
  def viewWithUnAuthenticatedAgent(test: Future[Result] => Any) {
    ...
    val result = TestRejectClientController.view("1").apply(SessionBuilder.buildRequestWithSessionNoUser)
    test(result)
  }
  ...
  def showConfirmationWithAuthorisedAgent(test: Future[Result] => Any) {
    ...
    implicit val user = AuthBuilder.createOrgAuthContext(userId, "name")
    AuthBuilder.mockAuthorisedAgent(userId, mockAuthConnector)
    val result = TestRejectClientController.showConfirmation("Acme Ltd").apply(SessionBuilder.buildRequestWithSession(userId))
    test(result)
  }
  ...
  def submitWithAuthorisedAgent(request: FakeRequest[AnyContentAsFormUrlEncoded])(test: Future[Result] => Any) {
    ...
    AuthBuilder.mockAuthorisedAgent(userId, mockAuthConnector)
    val result = TestRejectClientController.confirm("1", "Acme Ltd").apply(SessionBuilder.updateRequestFormWithSession(request, userId))
    test(result)
  }

  "RejectClientController" must {
    "not return NOT_FOUND at route " when {
      "GET /mandate/agent/reject-client/1" in {
        val result = route(FakeRequest(GET, "/mandate/agent/reject-client/1")).get
        status(result) mustNot be(NOT_FOUND)
      }

      "POST /mandate/agent/reject-client/1" in {
        val result = route(FakeRequest(POST, s"/mandate/agent/reject-client/1/Acme")).get
        status(result) mustNot be(NOT_FOUND)
      }
    }
    ...
    "return 'reject client question' view for AUTHORISED agent" when {
      ...
      "agent requests(GET) for 'reject client question' view" in {
        ...
        val mandate = Mandate(id = "1", createdBy = User("credId", "agentName", Some("agentCode")), None, None, agentParty = Party("JARN123456", "agency name", PartyType.Organisation, ContactDetails("agent@agent.com", None)), clientParty = Some(Party("JARN123456", "ACME Limited", PartyType.Organisation, ContactDetails("client@client.com", None))), currentStatus = MandateStatus(Status.New, DateTime.now(), "credId"), statusHistory = None, Subscription(None, Service("ated", "ATED")))

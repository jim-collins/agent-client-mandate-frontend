    ...
    "return 'nrl question' view for AUTHORISED agent" when {
      "agent requests(GET) for 'client permission' view from PaySA" in {
        viewWithAuthorisedAgent(service, PaySAQuestionController.controllerId) { result =>
          ...
          document.getElementById("permission-text").text() must be("Your client must complete an ATED 1, if you already have an ATED 1 for this client, don't complete another. You do not need to send your ATED 1 to HMRC, but you need to keep it for your records. Submission of form 64-8 does not cover ATED or ATED related Capital Gains Tax (CGT).")

...
class CollectAgentEmailControllerSpec extends PlaySpec with OneServerPerSuite with MockitoSugar with BeforeAndAfterEach {
    ...
    "returns BAD_REQUEST" when {
      "empty form is submitted" in {
        ...
        submitEmailAuthorisedAgent(fakeRequest) { result =>
          ...
          document.getElementsByClass("error-notification").text() must include("You must answer the email address question. You must answer the email address question.")
  ...
  def viewEmailUnAuthenticatedAgent(redirectUrl: Option[String]  = None)(test: Future[Result] => Any) {
    val userId = s"user-${UUID.randomUUID}"
    implicit val hc: HeaderCarrier = HeaderCarrier()
    AuthBuilder.mockUnAuthenticatedClient(userId, mockAuthConnector)
    val result = TestCollectAgentEmailController.view(service, redirectUrl).apply(SessionBuilder.buildRequestWithSessionNoUser)
    test(result)
  }

  def viewEmailUnAuthorisedAgent(redirectUrl: Option[String]  = None)(test: Future[Result] => Any) {
    val userId = s"user-${UUID.randomUUID}"
    implicit val hc: HeaderCarrier = HeaderCarrier()

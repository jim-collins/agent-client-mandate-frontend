    ...
    "return 'what is your email address' for AUTHORISED agent" when {
      ...
      "agent requests(GET) for 'what is your email address' view and the data hasn't been cached" in {
        viewEmailAuthorisedAgent() { result =>
          ...
          document.getElementById("info").text() must be(s"We will use this email address to send you notifications about this client. We won’t send confidential information by email. You can use a group email address and change it.")
